"""DMI Greenland station observation handler.

Reads a YAML config containing a ``stations:`` list, then for each active
station fetches observations from the DMI Open Data API and persists them as
CSV files (one per station, named ``<station_id>.csv``).
...
from ingestion.sources.dmi import fetch_dmi
...
def run(config_path: Path, output_dir: Path) -> None:
    ...
    # DMI Open Data no longer requires authentication (as of Dec 2025).
    # The api_key field in secrets is ignored but harmless if present.
