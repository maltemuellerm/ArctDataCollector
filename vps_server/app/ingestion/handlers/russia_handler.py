"""Russia / FJL / Novaya Zemlya station handler (NCEI ISD).

Reads russia_stations.yaml and fetches observations from the NCEI
global-hourly dataset for each active station.

On each run:
- If the station CSV is empty / missing : fetch from CAMPAIGN_START to now.
- If existing rows are present          : fetch from latest timestamp to now.
"""
...
from ingestion.sources.russia import fetch_russia

logger = logging.getLogger(__name__)

# Source is now OGIMET SYNOP (~1-6h lag). CAMPAIGN_START filters existing rows.
# OGIMET only retains ~58 days; the fetcher caps requests automatically.
